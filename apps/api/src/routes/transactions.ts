...
app.post('/upload', async (c) => {
    ...
    try {
        ...
        if (!isImage && !isPdf) {
            return c.json({ error: 'Invalid file type. Only images and PDFs are allowed.' }, 400)
        }

        const fileName = `${randomUUID()}${ext}`
        const userId = user.id
        const uploadDir = path.join(process.cwd(), 'uploads', userId, 'transactions')
...
app.patch('/:id', zValidator('json', createTransactionSchema.partial()), async (c) => {
    const user = c.get('user')
    const id = c.req.param('id')
    const body = c.req.valid('json')
